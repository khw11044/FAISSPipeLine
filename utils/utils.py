...
################################## 2. 임베딩모델 바꿔보기 ##########################################################
def get_embedding(model_path="intfloat/multilingual-e5-base"):
    # 임베딩 모델 설정
...
def create_chroma_db(pdf_title, chunks, model_path="intfloat/multilingual-e5-base"):
    """Chroma DB 생성"""
    # 임베딩 모델 설정
...
################################## 3. Chroma로 할건지, FAISS로 할건지 등 ##########################################################
def create_faiss_db(save_file, chunks, model_path="intfloat/multilingual-e5-base"):
    """Chroma DB 생성"""
    # 임베딩 모델 설정
    embeddings = get_embedding(model_path=model_path)
    # Chroma DB 생성 및 반환 persist_directory=DB_PATH
...
def process_pdfs_from_dataframe_faiss(df, base_directory, save_path, file_mapping):
    ...
    for path in tqdm(unique_paths, desc="Processing PDFs"):
        ...
        create_faiss_db(save_file, chunks, model_path="intfloat/multilingual-e5-base")
        with open(f'{save_file}.pkl', 'wb') as f:
            pickle.dump(chunks, f)